...
class TestEnrichmentSaveWrapper:
    ...
    def test_save_called_after_enrichment(self):
        ...
        with patch("app.routes.HistoryStore") as MockStore:
            ...
            _run_enrichment_and_save(
                mock_orch, "test_job_id", iocs, "test input", "online"
            )

            mock_orch.enrich_all.assert_called_once_with("test_job_id", iocs)
            mock_instance.save_analysis.assert_called_once()
            call_kwargs = mock_instance.save_analysis.call_args
    ...
    def test_save_failure_does_not_break_enrichment(self):
        ...
        with patch("app.routes.HistoryStore") as MockStore:
            mock_instance = MagicMock()
            mock_instance.save_analysis.side_effect = Exception("DB error")
            MockStore.return_value = mock_instance
    ...
    def test_save_skipped_when_status_none(self):
        ...
        with patch("app.routes.HistoryStore") as MockStore:
            ...
            mock_instance.save_analysis.assert_not_called()
...
class TestHistoryDetailRoute:
    ...
    def test_history_returns_200_with_seeded_data(self, client, seeded_store):
        ...
        with patch("app.routes.HistoryStore", return_value=store):
    ...
    def test_history_returns_404_for_unknown_id(self, client):
        ...
        with patch("app.routes.HistoryStore") as MockStore:
            mock_instance = MagicMock()
            mock_instance.load_analysis.return_value = None
            MockStore.return_value = mock_instance
...
class TestIndexWithHistory:
    ...
    def test_index_no_history(self, client):
        ...
        with patch("app.routes.HistoryStore") as MockStore:
            mock_instance = MagicMock()
            mock_instance.list_recent.return_value = []
            MockStore.return_value = mock_instance
    ...
    def test_index_history_error_graceful(self, client):
        ...
        with patch("app.routes.HistoryStore", side_effect=Exception("DB error")):
            response = client.get("/")
        assert response.status_code == 200
        assert b"Recent Analyses" not in response.data
