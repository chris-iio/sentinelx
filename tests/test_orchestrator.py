...
def _make_result(ioc: IOC) -> EnrichmentResult:
...
def _make_error(ioc: IOC, msg: str = "Timeout") -> EnrichmentError:
    return EnrichmentError(ioc=ioc, provider="VirusTotal", error=msg)


def _make_orchestrator(adapter, max_workers: int = 4) -> EnrichmentOrchestrator:
    return EnrichmentOrchestrator(adapter=adapter, max_workers=max_workers)
...
class TestEnrichAll:
    ...
    def test_enrich_all_calls_lookup_for_each_enrichable_ioc(self, mock_adapter):
        """CVE is not in ENDPOINT_MAP — adapter.lookup must NOT be called for it."""
...
class TestLRUEviction:
    ...
    def test_job_cleanup_lru(self, mock_adapter):
        ...
        orchestrator = EnrichmentOrchestrator(adapter=mock_adapter, max_workers=1, max_jobs=5)
