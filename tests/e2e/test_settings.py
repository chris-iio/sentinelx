...
Covers: page rendering, provider listing, API key forms, status indicators,
show/hide toggle (JS), flash messages, CSRF, navigation, security headers.

All tests use an isolated config directory (via _isolate_config fixture in conftest)
so no real API keys at ~/.sentinelx/config.ini are read or modified.
"""
...
# The 5 key-requiring providers displayed on the settings page.
# Public providers (MalwareBazaar, ThreatFox, Shodan InternetDB) are omitted
# because they need no configuration.
EXPECTED_PROVIDERS = ["virustotal", "urlhaus", "otx", "greynoise", "abuseipdb"]
...
def test_settings_page_title_tag(page: Page, live_server: str) -> None:
    ...
    # base.html sets <title>sentinelx</title>
...
def test_settings_lists_all_providers(page: Page, live_server: str) -> None:
    """All 5 key-requiring providers are listed as sections."""
    sp = SettingsPage(page, live_server)
    sp.goto()
...
def test_each_provider_has_description(page: Page, live_server: str) -> None:
    """Each provider section shows a description of what the provider covers."""
    sp = SettingsPage(page, live_server)
    sp.goto()
...
def test_providers_show_not_configured_by_default(page: Page, live_server: str) -> None:
    ...
    This works because E2E tests use an isolated empty config directory.
    """
    sp = SettingsPage(page, live_server)
    sp.goto()
...
def test_show_hide_toggle_hides_key_again(page: Page, live_server: str) -> None:
    ...
    pid = "greynoise"
    sp.toggle_key_visibility(pid)  # Show
    sp.expect_key_input_visible(pid)

    sp.toggle_key_visibility(pid)  # Hide
    sp.expect_key_input_masked(pid)
    expect(sp.show_hide_button(pid)).to_have_text("Show")
...
def test_empty_key_shows_error_flash(page: Page, live_server: str) -> None:
    ...
    No config is modified — the server rejects empty keys before saving.
    """
    sp = SettingsPage(page, live_server)
    sp.goto()

    # Clear input and submit empty key
...
def test_save_key_shows_success_flash(page: Page, live_server: str) -> None:
    ...
    Uses the isolated temp config directory so this doesn't touch real keys.
    """
    sp = SettingsPage(page, live_server)
    sp.goto()
...
def test_saved_key_is_masked_on_reload(page: Page, live_server: str) -> None:
    """After saving a key, reloading shows a masked version (asterisks + last 4 chars)."""
    sp = SettingsPage(page, live_server)

    # Save a key first (self-contained — does not depend on other tests)
    sp.goto()
    ...
    assert "****" in input_val or "***" in input_val, f"Masked key should contain asterisks, got: {input_val}"
