...
**Goal:** Close M012 with auditable validation evidence by filling the missing slice-assessment artifacts, re-running the focused continuity proof that still matters, and rendering the canonical milestone validation artifact from the real evidence surface instead of leaving closeout claims implicit.
**Demo:** After this: M012 has explicit proof or justified re-scope for every claimed continuity requirement, slice-level assessment coverage is complete or formally omitted with rationale, and the S04 persistence/helper findings are tied into the milestone closeout so validation can pass.

## Must-Haves

- Create file-backed assessment coverage for `S01`, `S02`, and `S03` so milestone validation no longer depends on missing slice-local assessment artifacts.
- Use `.gsd/REQUIREMENTS.md` plus the existing `S01`–`S04` summaries/assessments as the proof index for continuity requirements `R008`, `R009`, `R010`, `R014`, `R015`, `R018`, `R019`, `R020`, `R022`, and `R040`.
- Re-run focused proof for orchestrator, API/routes/http-safety, adapter-session, and cache/history/settings continuity, plus `make verify-fast`, before issuing any milestone verdict.
- Call `gsd_validate_milestone` so `.gsd/milestones/M012/M012-VALIDATION.md` exists with a truthful pass-or-remediation verdict, explicit slice delivery audit, and cross-slice integration coverage.
...
- Upstream surfaces consumed: `.gsd/REQUIREMENTS.md`, `.gsd/milestones/M012/M012-ROADMAP.md`, `.gsd/milestones/M012/M012-CONTEXT.md`, `.gsd/milestones/M012/slices/S01/S01-SUMMARY.md`, `.gsd/milestones/M012/slices/S02/S02-SUMMARY.md`, `.gsd/milestones/M012/slices/S03/S03-SUMMARY.md`, `.gsd/milestones/M012/slices/S04/S04-SUMMARY.md`, `.gsd/milestones/M012/slices/S04/S04-ASSESSMENT.md`, and the focused pytest / `make verify-fast` command surfaces.
- New wiring introduced in this slice: no product-code wiring; only canonical artifact writes through `gsd_summary_save` for the missing slice assessments and `gsd_validate_milestone` for the milestone validation artifact.
- What remains before the milestone is truly usable end-to-end: after a passing validation artifact is rendered, only milestone completion remains; if validation finds a real gap, it must become explicit remediation rather than hidden drift.
...
- `python3 -m pytest tests/test_orchestrator.py tests/test_api.py tests/test_routes.py tests/test_http_safety.py tests/test_adapter_contract.py -q`
...
- [x] **T01: Create the missing slice assessment artifacts for S01–S03** `est:0.25d`
...
- [ ] Any uncovered gap becomes explicit remediation or a blocked verdict instead of being hidden behind milestone-closeout prose.
  - Files: `.gsd/REQUIREMENTS.md`, `.gsd/milestones/M012/M012-ROADMAP.md`, `.gsd/milestones/M012/M012-CONTEXT.md`, `.gsd/milestones/M012/slices/S01/S01-SUMMARY.md`, `.gsd/milestones/M012/slices/S02/S02-SUMMARY.md`, `.gsd/milestones/M012/slices/S03/S03-SUMMARY.md`, `.gsd/milestones/M012/slices/S04/S04-SUMMARY.md`, `.gsd/milestones/M012/slices/S04/S04-ASSESSMENT.md`, `.gsd/milestones/M012/M012-VALIDATION.md`, `Makefile`
