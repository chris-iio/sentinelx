...
| status-snapshot-scaling | `internal benchmark: EnrichmentOrchestrator.get_status() snapshot scaling` | 0 | 93 | 400 polls at 5000 retained results: `get_status()` 89.17ms vs `get_incremental_status(since=4990)` 4.02ms (22.2x faster) while returning 10 tail rows with next_since=5000. |
| cache-store-tempdb | `internal benchmark: CacheStore temp WAL put/get loop` | 0 | 18 | Temp WAL cache DB: 250 puts in 4.33ms, 250 TTL reads in 1.20ms, 250 hits, 250 retained rows. |
...
| history-store-tempdb | `internal benchmark: HistoryStore temp WAL save/list/load loop` | 0 | 16 | Temp WAL history DB: 180 saves in 3.82ms, list_recent(20) in 0.05ms, single load in 0.02ms, latest total_count=1, recent rows=20. |
| pipeline-duplicate-candidates | `internal benchmark: run_pipeline normalized duplicate candidate gate` | 0 | 68 | 7 raw URL variants normalize to 1 IOC value(s); classify calls=1; output values=http://evil.com. |
...
| Finding | Seam | Evidence kind | Evidence summary | Continuity guardrails | Rerun lanes | Continuity notes |
| --- | --- | --- | --- | --- | --- | --- |
| Leave provider concurrency/backoff semantics alone during M020 unless fresh live evidence contradicts the M017 keep-decision. | runtime/provider | measurement + code-path reasoning | The M017 `runtime-provider-diagnostics` capture showed cache hits did not dominate dispatch cost, while tests protect semaphore scope, 429 backoff, cached markers, and diagnostics. `app/enrichment/retry_policy.py` now owns retry constants, 429/rate-limit classification, and exponential delay math; `app/enrichment/lookup_execution.py::run_lookup_with_retries()` now owns the retry/backoff loop while preserving the existing contract that sleeps happen after each semaphore-scoped attempt has returned. `app/enrichment/lookup_execution.py::run_attempt_with_semaphore()` now owns optional provider semaphore acquire/release try/finally mechanics, and `run_lookup_with_retries()` invokes that boundary directly around each bound single-attempt callback instead of keeping an orchestrator compatibility delegator. Focused proof lives in `tests/test_orchestrator.py::TestBackoff429::test_orchestrator_delegates_retry_policy_helpers`, `test_orchestrator_delegates_retry_execution_loop`, and `test_orchestrator_delegates_semaphore_attempt_boundary`, while the existing backoff and semaphore tests keep the provider scheduling contract intact. M020 should not rewrite provider scheduling simply because it is complex; complexity here reflects quota and failure semantics. | R014, R015, R018, R020, R040, R099 | `python3 -m pytest -q tests/test_orchestrator.py`; `make verify-deep` for live-enrichment-visible changes | Preserve per-provider concurrency caps, retry/backoff, adapter-owned sessions, cache-hit markers, terminal failure visibility, and diagnostics unless a new measurement proves a better contract; keep retry policy constants/classification centralized, keep backoff sleeps outside semaphore-scoped lookup attempts, and avoid embedding semaphore acquire/release mechanics in the orchestrator class. |
