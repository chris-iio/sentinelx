...
- Mode: `baseline`
- Generated at: `2026-05-16 08:42:32 UTC`
...
| --- | --- | ---: | ---: | --- |
| runtime-provider-diagnostics | `internal benchmark: EnrichmentOrchestrator synthetic runtime/provider diagnostics` | 0 | 3 | provider mix CacheAlpha:2d/0e, RateLimitBeta:2d/1e; dispatch=4; attempts=5; cache-hit ratio 1/5 (20%); retries=1 (429=1); errors=1; latency total=2.25s max=1.00s. |
| status-snapshot-scaling | `internal benchmark: EnrichmentOrchestrator.get_status() snapshot scaling` | 0 | 32 | 400 polls at 5000 retained results: `get_status()` 29.43ms vs `get_incremental_status(since=4990)` 3.25ms (9.1x faster) while returning 10 tail rows with next_since=5000. |
| cache-store-tempdb | `internal benchmark: CacheStore temp WAL put/get loop` | 0 | 24 | Temp WAL cache DB: 250 puts in 7.09ms, 250 TTL reads in 1.69ms, 250 hits, 250 retained rows. |
...
| history-store-tempdb | `internal benchmark: HistoryStore temp WAL save/list/load loop` | 0 | 18 | Temp WAL history DB: 180 saves in 5.88ms, list_recent(20) in 0.07ms, single load in 0.03ms, latest total_count=1, recent rows=20. |
| pipeline-duplicate-candidates | `internal benchmark: run_pipeline normalized duplicate candidate gate` | 0 | 93 | 7 raw URL variants normalize to 1 IOC value(s); classify calls=1; output values=http://evil.com. |
