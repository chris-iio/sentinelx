...
- Mode: `baseline`
- Generated at: `2026-05-16 08:50:43 UTC`
...
| status-snapshot-scaling | `internal benchmark: EnrichmentOrchestrator.get_status() snapshot scaling` | 0 | 37 | 400 polls at 5000 retained results: `get_status()` 34.36ms vs `get_incremental_status(since=4990)` 2.93ms (11.7x faster) while returning 10 tail rows with next_since=5000. |
| cache-store-tempdb | `internal benchmark: CacheStore temp WAL put/get loop` | 0 | 17 | Temp WAL cache DB: 250 puts in 3.61ms, 250 TTL reads in 1.08ms, 250 hits, 250 retained rows. |
...
| history-store-tempdb | `internal benchmark: HistoryStore temp WAL save/list/load loop` | 0 | 18 | Temp WAL history DB: 180 saves in 4.03ms, list_recent(20) in 0.08ms, single load in 0.05ms, latest total_count=1, recent rows=20. |
| pipeline-duplicate-candidates | `internal benchmark: run_pipeline normalized duplicate candidate gate` | 0 | 82 | 7 raw URL variants normalize to 1 IOC value(s); classify calls=1; output values=http://evil.com. |
