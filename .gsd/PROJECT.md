...
**M014 is complete.** SentinelX now has a hardened local workflow and recovery loop around repo-local runtime state, supported repair, and local dev-server lifecycle management. The product-facing SentinelX stack remains stable: the checked-in M013 optimization-audit workflow, runtime/provider keep-decisions, WAL persistence seams, request/status hot-path improvement, frontend/render coordinator optimization, and final verification lanes all remain intact.

M014 closed the local workflow hardening pass around the repo/operator seam rather than an analyst-facing feature. S01 established the authoritative runtime-state boundary: `tools/runtime_state_boundary.py` classifies durable `.gsd` artifacts, transient `.gsd`/`.bg-shell` runtime state, and fail-closed legacy `.planning/**` paths; `.gitignore` and the repo-native verifier were aligned to that contract; and temp-repo Git regression fixtures prove the stash/pop blocker class is either prevented or surfaced explicitly. S02 added the supported recovery loop: `tools/runtime_state_repair.py` is the only mutating repair CLI, `make repair-runtime-state` is the one supported repo-native recovery entrypoint, `tracked-transient` findings are deindexed, `unignored-transient` findings are quarantined under `.gsd/runtime/repair-quarantine/<timestamp>/...`, and live-repo repair/audit runs stay green while `.planning/**` remains visible manual-review backlog instead of being auto-cleaned. S03 standardized the supported local server lifecycle: `tools/dev_server.py` plus `make dev-server-start|status|restart|stop` are the only supported local dev loop, manager-owned state stays under `.gsd/runtime/dev-server/**`, `/api/health` is a fixed secret-free readiness probe, and fresh slice verification plus a live crash/restart exercise validated cheap local recovery without widening the runtime boundary. S04 then closed the seam with an explicit review/refactor pass and final assembly proof: `.gsd/milestones/M014/slices/S04/S04-REVIEW.md` records the reviewed invariants, `app/health_contract.py` is the single source of truth for the local health contract consumed by both `app/routes/api.py` and `tools/dev_server.py`, `.gsd/milestones/M014/slices/S04/S04-LIFECYCLE-PROOF.json` captures the real start → healthy → forced crash → crashed → restart → stop operator transcript, and `.gsd/milestones/M014/slices/S04/S04-CLOSURE-PROOF.md` records final proof.

Milestone completion re-verified the assembled final state: the focused runtime/repair/dev-server suite, `make repair-runtime-state`, `make verify-runtime-boundary`, and full `make verify` all exited 0. `make verify` reported `1018` non-E2E pytest passes, `81` Vitest passes, clean TypeScript/build, and `113` E2E passes. `tools/optimization_audit.py`, `Makefile`, `README.md`, and `docs/optimization-audit.md` remain the command surface and proof vocabulary for performance-related work; M014 hardened the local developer/operator loop around them so repo-local state and crashed local processes are easier to classify, repair, inspect, and restart without risking milestone/context artifacts.
...
- **Build/dev entrypoints:** Makefile targets include `css`, `js`, `js-dev`, `js-watch`, `typecheck`, `build`, `dev-server-start`, `dev-server-status`, `dev-server-restart`, `dev-server-stop`, `repair-runtime-state`, `verify-runtime-boundary`, `verify-fast`, `verify-deep`, `verify`, `audit-m013-template`, and `audit-m013`
...
- **Shared health contract seam:** `app/health_contract.py` is the single source of truth for `HEALTH_PATH` and `HEALTH_PAYLOAD`; `app/routes/api.py` produces that contract and `tools/dev_server.py` consumes it with fail-closed exact-match semantics so drift or secret-bearing extra keys surface as `malformed`
...
---
*Last updated: 2026-04-26 — M014 complete with local workflow hardening, recovery tooling, dev-server crash recovery, and final verification recorded.*
