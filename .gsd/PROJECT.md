...
**M015 is active: Intake Workbench. S01 and S02 are complete.** All prior milestones through M014 are complete. The product-facing SentinelX stack is stable: extraction/enrichment, result rendering, detail pages, history reload, REST API, optimization-audit workflow, fast/deep verification lanes, and local workflow recovery are in place and verified.
...
The current product-facing gap is the first screen. The results/detail/history surfaces are mature, but `/` is being redesigned into a fast analyst **Intake Workbench** without changing extraction/enrichment semantics. The user explicitly chose **go fast**, no pre-submit extraction preview, and a **compact list** of Recent Analyses on the intake page. M015/S01 shipped the foundational command-card intake surface: the paste form is visually dominant, the stable form IDs and hidden offline `mode` contract are preserved, and browser proof confirms offline paste-to-results still works. M015/S02 clarified the Offline/Online mode UI inside that command card: visible help/status copy now explains the default offline safety path and online provider/enrichment intent, while `form.ts` keeps the hidden mode input, widget data attribute, ARIA state, status copy, and submit-button mode class synchronized. Remaining M015 slices add the compact Recent Analyses rail with graceful history failure handling (S03) and run the integrated desktop/mobile/full-regression proof (S04).
...
- **M015 intake command surface:** `/` now uses `.page-index`, `.intake-workbench`, and `.command-card` as the S01 command-card foundation. Later intake slices should build around those wrappers without renaming the stable form controls or adding pre-submit IOC parsing.
...
- [ ] M015: Intake Workbench — S01 completed the command-card intake foundation and S02 completed the clarified Offline/Online mode UI with preserved hidden-input semantics; S03 remains to add compact Recent Analyses with graceful degradation, and S04 remains to prove the integrated workbench.

---
*Last updated: 2026-04-26 — M015 active; S01 command-card intake foundation and S02 clarified mode UI complete.*
